...
class Annotator:
    ...
    def __init__(self, im, line_width=None, font_size=None, font="Arial.ttf", pil=False, example="abc"):
        ...
        if self.pil:  # use PIL
            ...
            try:
                font_path = "Arial.Unicode.ttf" if non_ascii else font
                size = font_size if font_size else max(round(sum(self.im.size) / 2 * 0.035),
                                                       12)  # use font_size if provided
            ...
            self.sf = font_size / 10 if font_size else self.lw / 3  # use font_size to adjust cv2 font scale if provided
        # Pose
        self.skeleton = [
            [16, 14],
            [14, 12],
